...
mixin(Request.prototype, {
   ...
   _fireSuccess: function(body, response) {
      if (parseInt(response.statusCode) >= 400) {
         if (this.options.callback) {
            this.options.callback.call(this, body, null, response);
         }
      } else {
         if (this.options.callback) {
...
function setCallbackOption(options, callback) {
...
function shortcutOptions(options, callback, method) {
   options = setCallbackOption(options, callback);
...
function get(url, options, callback) {
 return request(url, shortcutOptions(options, callback, "GET"));
}

function patch(url, options, callback) {
   return request(url, shortcutOptions(options, callback, "PATCH"));
}

function post(url, options, callback) {
   return request(url, shortcutOptions(options, callback, "POST"));
}

function put(url, options, callback) {
   return request(url, shortcutOptions(options, callback, "PUT"));
}

function del(url, options, callback) {
   return request(url, shortcutOptions(options, callback, "DELETE"));
}

function head(url, options, callback) {
   return request(url, shortcutOptions(options, callback, "HEAD"));
}

function json(url, data, options, method, callback) {
   if (typeof options === "function") {
      options = { callback: options, method: "POST" };
   } else if (typeof options === "string") {
      options = { callback: method, method: options };
   } else {
      options = options || {};
      if (typeof method === "function") {
         options.method = "POST";
         if (method) options.callback = method;
      } else {
         options.method = method;
...
function postJson(url, data, options, callback) {
   if (typeof options === "function") {
      callback = options;
      options = {};
   }

   return json(url, data, options, 'POST', callback);
}
